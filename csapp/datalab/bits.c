 ...
 */
 int bitXor(int x, int y) {
      ...
      * |---+---+-----|
      *
      * 1. The truth table shows the fact that x^y = (x & ~y) | (~x & y)
      * 2. According to De Morgan's Law, we have:
      *      a. ~(x | y) = (~x) & (~y)
      *      b. ~(x & y) = (~x) | (~y)
      * 3. Hence x^y = (x & ~y) | (~x & y)
      *              = ~~((x & ~y) | (~x & y))
...
int isTmax(int x) {
    ...
    //
    // Note that `(x + 1) ^ (~x) == 0` is still true when `x == -1`, 
    // so we also need to return `0` when `x + 1` is 0.
...
int allOddBits(int x) {
    // 0xAA: 1010 1010
    // (0xAA << 8) | 0xAA: 1010 1010 1010 1010
    int m16 = (0xAA << 8) | 0xAA;
    int m32 = (m16 << 16) | m16;
